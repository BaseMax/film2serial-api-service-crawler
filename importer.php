...
class Film2Serial {
    ...
    function alt($text) {
        return html_entity_decode(preg_replace('/([\-]{2,})/i', '-', preg_replace('/([\s\t\n]+)/i', '-', strip_tags($text))));
    }
...
// 			"alt_name"=>preg_replace('/([\-]{2,})/i', '-', preg_replace('/([\s\t\n]+)/i', '', $title)),
			...
			"descr"=>mb_substr(strip_tags($context), 0, 299, "utf-8"),
// 			"descr"=>substr_replace(strip_tags($context), "...", 299),
		...
		if($db->count("sld_post", ["title"=>$title]) == 0) {
    		$postID=$db->insert("sld_post", $data);
		}
		else {
		    $post=$db->select("sld_post", ["title"=>$title], "", "id");
		    $postID=$post["id"];
		}
		$_vals=[
		    "eid"=>null,
		...
		if($data["category"] != "") {
		    foreach(explode(",", $data["category"]) as $category) {
		        $clauses=["news_id"=>$postID, "cat_id"=>$category];
		        print_r($clauses);
        		if($db->count("sld_post_extras_cats", $clauses) == 0) {
    		        $db->insert("sld_post_extras_cats", $clauses);
        		}
		    }
		}
		return $data;
